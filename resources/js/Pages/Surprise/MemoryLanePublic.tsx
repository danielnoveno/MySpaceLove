...
export default function MemoryLanePublic({
    ...
    const puzzleOptions = memoryLane.puzzle.options;
    const safePuzzleOptions =
        puzzleOptions.length > 0
            ? puzzleOptions
            : [{ label: "", value: "" }];
    const [puzzleImageIndex, setPuzzleImageIndex] = useState(0);
    const activePuzzleOption =
        safePuzzleOptions[puzzleImageIndex] ?? safePuzzleOptions[0];
    const puzzleImageUrl = activePuzzleOption?.value ?? safePuzzleOptions[0]?.value;
    const renderPuzzleButtons = () =>
        safePuzzleOptions.map((option, index) => {
            ...
            >
                <div className="relative mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-16 sm:px-8 md:px-12">
                    ...
                    <JigsawPuzzleGate
                        ...
                        title={memoryLane.puzzle.title}
                        description={memoryLane.puzzle.description}
                        solvedTitle={memoryLane.puzzle.solvedTitle}
                        solvedDescription={memoryLane.puzzle.solvedDescription}
