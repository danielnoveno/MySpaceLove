...
export default function EditWishlistPage() {
  ...
  useEffect(() => {
    ...
    if (user && slug && itemId) {
      ;(async () => {
        ...
        const { data: item } = await supabase.from('wishlist_items').select('*').eq('id', itemId).eq('space_id', space.id).single()
        if (!item) { router.push(`/spaces/${slug}/wishlist`); return }
        setTitle(item.title || ''); setDescription(item.description || '')
        setLocation(item.location || ''); setNotes(item.notes || '')
        setLoading(false)
      })()
    }
  }, [user, authLoading, router, slug, itemId, supabase])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (updateError) { setError('Gagal memperbarui. Silakan coba lagi.'); setSaving(false); return }
  ...
  return (
    <AuthenticatedLayout header={
      <FadeIn><div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-warm-900">Edit Wishlist</h1><p className="text-warm-500">Perbarui item wishlist Anda</p></div>
      </div></FadeIn>
    }>
