  ...
  useEffect(() => {
    ...
    if (user && slug && countdownId) {
      ;(async () => {
        ...
        const { data: countdown } = await supabase.from('countdowns').select('*').eq('id', countdownId).eq('space_id', space.id).single()
        if (!countdown) { router.push(`/spaces/${slug}/countdowns`); return }
        setName(countdown.name); setEventDate(countdown.event_date ? countdown.event_date.split('T')[0] : '')
        setDescription(countdown.description || ''); setActivities(countdown.activities || ''); setLoading(false)
      })()
    }
  }, [user, authLoading, router, slug, countdownId, supabase])

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (!name.trim() || !eventDate) return; setSaving(true); setError('')
    const { error: updateError } = await supabase.from('countdowns').update({ name: name.trim(), event_date: eventDate, description: description.trim() || null, activities: activities.trim() || null }).eq('id', countdownId)
    if (updateError) { setError('Failed to update countdown. Please try again.'); setSaving(false); return }
  ...
  return (
    <AuthenticatedLayout header={
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-bold text-warm-900">Edit Countdown</h1><p className="text-warm-500">Update your countdown details</p></div>
      </div>
    }>
        ...
        <div className="rounded-3xl bg-white border border-warm-100 p-6 shadow-xl shadow-warm-900/5">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <Link href={`/spaces/${slug}/countdowns`} className="rounded-xl px-6 py-3 text-sm font-medium text-warm-700 hover:bg-warm-100 transition-colors">Cancel</Link>
              <button type="submit" disabled={saving || !name.trim() || !eventDate}
                ...
                {saving ? <><Loader2 className="h-4 w-4 animate-spin" />Saving...</> : <><Save className="h-4 w-4" />Update Countdown</>}
              </button>
            </div>
          </form>
        </div>
