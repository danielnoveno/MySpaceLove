...
type ExistingPhoto = { id: string; url: string; caption: string | null }
...
export default function EditGalleryPage() {
  ...
  useEffect(() => {
    ...
    if (user && slug && collectionId) {
      ;(async () => {
        ...
        const { data: collection } = await supabase.from('gallery_collections').select('*').eq('id', collectionId).eq('space_id', space.id).single()
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setNewFiles((prev) => [...prev, ...selected.map((file) => ({ file, preview: URL.createObjectURL(file) }))])
  }

  const removeNewFile = (index: number) => { setNewFiles((prev) => { URL.revokeObjectURL(prev[index].preview); return prev.filter((_, i) => i !== index) }) }
  const markExistingPhotoDeleted = (photoId: string) => { setExistingPhotos((prev) => prev.filter((p) => p.id !== photoId)); setDeletedPhotoIds((prev) => [...prev, photoId]) }

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); if (existingPhotos.length === 0 && newFiles.length === 0) { setError('A collection must have at least one photo.'); return }
    setSaving(true); setError(''); setUploadProgress(0)
    try {
      const { error: updateError } = await supabase.from('gallery_collections').update({ title: title.trim() || 'Untitled Collection' }).eq('id', collectionId)
      if (updateError) throw updateError
      if (deletedPhotoIds.length > 0) {
        for (const photoId of deletedPhotoIds) { const photo = existingPhotos.find((p) => p.id === photoId); if (photo) { const urlParts = photo.url.split('/gallery/'); if (urlParts.length > 1) await supabase.storage.from('gallery').remove([urlParts[1]]) } }
        const { error: deleteError } = await supabase.from('gallery_photos').delete().in('id', deletedPhotoIds); if (deleteError) throw deleteError
      }
      if (newFiles.length > 0) {
        ...
        for (let i = 0; i < newFiles.length; i++) {
          ...
          const filePath = `${collectionId}/${Date.now()}-${Math.random().toString(36).slice(2, 8)}.${fileExt}`
        ...
        if (uploadedUrls.length > 0) {
          const { error: photoError } = await supabase.from('gallery_photos').insert(uploadedUrls.map((url, idx) => ({ collection_id: collectionId, url, caption: newFiles[idx].file.name.replace(/\.[^/.]+$/, '') })))
  ...
  return (
    <AuthenticatedLayout header={
      <div className="flex items-center gap-4">
        ...
        <div><h1 className="text-2xl font-bold text-warm-900">Edit Collection</h1><p className="text-warm-600">Update your photo collection</p></div>
      </div>
    }>
      <div className="max-w-2xl mx-auto">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {existingPhotos.length > 0 && (
            <div className="rounded-3xl bg-white border border-warm-100 p-6 shadow-xl shadow-warm-900/5">
              <label className="block text-sm font-medium text-warm-700 mb-2">Existing Photos ({existingPhotos.length})</label>
          ...
          <div className="rounded-3xl bg-white border border-warm-100 p-6 shadow-xl shadow-warm-900/5">
            <label className="block text-sm font-medium text-warm-700 mb-2">Add New Photos</label>
            <input ref={fileInputRef} type="file" accept="image/*" multiple onChange={handleFileSelect} className="hidden" />
            <button type="button" onClick={() => fileInputRef.current?.click()}
              className="w-full rounded-2xl border-2 border-dashed border-brand-200 bg-brand-50/50 py-8 text-center transition-all hover:border-brand-400 hover:bg-brand-50">
              <Upload className="h-8 w-8 text-brand-400 mx-auto mb-2" />
              <p className="font-medium text-warm-700 text-sm">Click to upload photos</p>
              <p className="text-xs text-warm-500 mt-1">PNG, JPG, GIF up to 10MB each</p>
            </button>
            {newFiles.length > 0 && (
            ...
            {newFiles.length > 0 && <p className="mt-3 text-sm text-warm-500">{newFiles.length} new photo{newFiles.length !== 1 ? 's' : ''} selected</p>}
          </div>
          {saving && (
            <div className="rounded-3xl bg-white border border-warm-100 p-6 shadow-xl shadow-warm-900/5">
              <div className="flex items-center gap-3 mb-3">
                <Loader2 className="h-5 w-5 text-brand-500 animate-spin" />
                <span className="text-sm font-medium text-warm-700">Saving... {uploadProgress}%</span>
              </div>
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button type="submit" disabled={saving || (existingPhotos.length === 0 && newFiles.length === 0)}
              ...
              {saving ? <><Loader2 className="h-4 w-4 animate-spin" />Saving...</> : <><Check className="h-4 w-4" />Update Collection</>}
            </button>
          </div>
        </form>
      </div>
