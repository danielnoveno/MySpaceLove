  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let i = 0; i < totalFiles; i++) {
        ...
        const filePath = `${spaceId}/${collection.id}/${fileName}`
      ...
      if (uploadedUrls.length > 0) {
        const photoInserts = uploadedUrls.map((url, idx) => ({
          ...
          url,
          caption: files[idx].file.name.replace(/\.[^/.]+$/, ''),
        }))
      ...
      header={
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-600">Upload photos to a new collection</p>
          </div>
        </div>
      }
    >
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="rounded-3xl bg-white/80 backdrop-blur shadow-sm border border-white/70 p-6">
            ...
            <p className="mt-2 text-xs text-gray-500">
              Leave blank to auto-name by upload date
            </p>
          </div>
          ...
          <div className="rounded-3xl bg-white/80 backdrop-blur shadow-sm border border-white/70 p-6">
            ...
            <button
              ...
              className="w-full rounded-2xl border-2 border-dashed border-pink-200 bg-pink-50/50 py-12 text-center transition-all hover:border-pink-400 hover:bg-pink-50"
            >
