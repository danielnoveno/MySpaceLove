  ...
  return (
        ...
        <FadeIn>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-warm-900">Notifikasi</h1>
                <p className="text-warm-500">
                  {space.title} — {unreadCount > 0 ? `${unreadCount} belum dibaca` : 'Semua sudah dibaca!'}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {isSelecting && selectedIds.size > 0 && (
                <button
                  ...
                  className="flex items-center gap-2 rounded-full bg-coral-50 px-4 py-2 text-sm font-medium text-coral-600 hover:bg-coral-100 transition-colors"
                >
              ...
              {isSelecting ? (
                <button
                  onClick={() => { setIsSelecting(false); deselectAll() }}
                  className="flex items-center gap-2 rounded-full bg-warm-100 px-4 py-2 text-sm font-medium text-warm-600 hover:bg-warm-200 transition-colors"
                >
                  Batal
                </button>
              ) : (
              ...
              {unreadCount > 0 && (
                <button
                  ...
                  className="flex items-center gap-2 rounded-full bg-brand-50 px-4 py-2 text-sm font-medium text-brand-600 hover:bg-brand-100 transition-colors"
                >
    ...
    >
      <div className="max-w-2xl mx-auto">
        {error && (
          <div className="mb-4 rounded-2xl bg-coral-50 border border-coral-200 p-4 text-sm text-coral-700">{error}</div>
        )}
        ...
        {isSelecting && notifications.length > 0 && (
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={deselectAll} className="text-sm text-warm-500 hover:text-warm-700 font-medium">
              Batal pilih
            </button>
          </div>
        )}
        ...
        {notifications.length === 0 ? (
          <FadeIn delay={0.1}>
            <div className="text-center py-16">
              ...
              <h2 className="text-xl font-semibold text-warm-900 mb-2">Tidak ada notifikasi</h2>
              <p className="text-warm-500">Belum ada notifikasi untuk ruang ini.</p>
            </div>
          </FadeIn>
        ) : (
            ...
            {notifications.map((notification) => {
              ...
              return (
                <StaggerItem key={notification.id}>
                  <div
                    className={`flex items-start gap-4 rounded-3xl p-4 transition-all cursor-pointer ${
                      isSelected
                        ? 'bg-brand-50 border-2 border-brand-300'
                        : notification.read
                          ? 'bg-warm-50 border border-warm-100'
                          : 'bg-white border border-warm-100 shadow-xl shadow-warm-900/5'
                    }`}
                  ...
                  >
                    {isSelecting && (
                      <div className="mt-1 shrink-0">
                        <div className={`h-5 w-5 rounded-full border-2 flex items-center justify-center transition-colors ${
                          isSelected ? 'bg-brand-500 border-brand-500' : 'border-warm-300'
                        }`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className={`text-sm font-medium ${notification.read ? 'text-warm-600' : 'text-warm-900'}`}>
                            {notification.title}
                          </p>
                      ...
                      <p className="text-xs text-warm-400 mt-2">
                        {new Date(notification.created_at).toLocaleDateString('id-ID')} pukul{' '}
