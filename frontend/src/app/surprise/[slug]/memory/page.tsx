...
const CATEGORY_CONFIG: Record<string, { label: string; icon: typeof Heart }> = {
  'first-date': { label: 'First Date', icon: Heart },
  anniversary: { label: 'Anniversary', icon: Star },
  trip: { label: 'Trip', icon: Plane },
  milestone: { label: 'Milestone', icon: Award },
  gift: { label: 'Gift', icon: Gift },
  adventure: { label: 'Adventure', icon: MapPin },
...
export default function PublicSurpriseMemoryPage() {
  ...
  const fetchMemory = useCallback(async () => {
    ...
    try {
      ...
      if (memError || !memoryData) {
        setError('No memories found to share')
        setLoading(false)
        return
      }
  ...
  const handlePinSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    setUnlocking(true)
    // In a real app, verify the PIN against the server
    ...
    return (
      <div className="surprise-page">
        ...
        <div className="surprise-content">
          ...
          <p className="text-white/80 mt-4 text-lg">Loading your surprise...</p>
        </div>
      </div>
    )
  }
  ...
  if (error || !memory) {
    return (
      <div className="surprise-page">
        <FloatingHearts />
        <div className="surprise-content">
          <div className="surprise-card text-center">
            <Heart className="h-16 w-16 text-pink-300 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Oops!</h2>
            <p className="text-gray-600">{error || 'Memory not found'}</p>
          </div>
        </div>
      </div>
    )
  ...
  if (pinRequired) {
    return (
      <div className="surprise-page">
        <FloatingHearts />
        <div className="surprise-content">
          <div className="surprise-card text-center max-w-sm mx-auto">
            <Lock className="h-12 w-12 text-pink-400 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-800 mb-2">This memory is locked</h2>
            <p className="text-gray-600 mb-6">Enter the PIN to view this surprise</p>
            <form onSubmit={handlePinSubmit} className="space-y-4">
              <input
                ...
                className="w-full text-center text-2xl tracking-[0.5em] rounded-xl border border-pink-200 bg-white/80 px-4 py-3 text-gray-900 placeholder-gray-400 focus:border-pink-400 focus:ring-2 focus:ring-pink-200 outline-none transition"
                autoFocus
              />
              {pinError && (
              ...
              <button
                ...
                className="w-full rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 px-6 py-3 text-white font-semibold shadow-lg transition hover:from-pink-600 hover:to-rose-600 disabled:opacity-50"
              >
  ...
  return (
    <div className="surprise-page">
      ...
      <div className="surprise-content">
        ...
        <div className="surprise-card animate-slide-up max-w-2xl mx-auto">
          ...
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-pink-100 text-pink-600 text-xs font-semibold px-3 py-1">
        ...
        <div className="text-center mt-10 animate-fade-in">
          <div className="flex items-center justify-center gap-2 text-white/60 text-sm">
            ...
            <span>Made with love by {space?.name || 'Someone Special'}</span>
            <Heart className="h-4 w-4 fill-current" />
          </div>
      ...
      <style jsx global>{`
        .surprise-page {
          min-height: 100vh;
          background: linear-gradient(135deg, #ec4899 0%, #a855f7 50%, #f43f5e 100%);
          display: flex;
          align-items: center;
