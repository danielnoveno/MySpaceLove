...
export default function PublicSurpriseStoryPage() {
  ...
  const loadChapters = useCallback(() => {
    setLoading(true)
    try {
      // Try to load from URL params first (base64 encoded)
      ...
      // Fall back to localStorage (same key as storybook page)
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          setError('No chapters found in the storybook')
        }
      } else {
        setError('No storybook found for this space')
      }
    } catch {
    ...
    return (
      <div className="story-page">
        ...
        <div className="story-content">
          ...
          <p className="text-white/80 mt-4 text-lg">Opening your storybook...</p>
        </div>
      </div>
    )
  }
  ...
  if (error || chapters.length === 0) {
    return (
      <div className="story-page">
        <FloatingHearts />
        <div className="story-content">
          <div className="story-card text-center max-w-md mx-auto">
            <BookOpen className="h-16 w-16 text-pink-300 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-gray-800 mb-2">No Story Yet</h2>
            <p className="text-gray-600">{error || 'The storybook is empty'}</p>
          </div>
        </div>
      </div>
    )
    ...
    <div className="story-page">
      ...
      <div className="story-content">
        ...
        <div className="story-card animate-slide-up max-w-2xl mx-auto">
          ...
          {/* Chapter Number */}
          <div className="text-xs font-semibold text-pink-500 uppercase tracking-widest mb-2">
            Chapter {activeChapter.chapter_number}
          </div>

          {/* Title */}
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-6 leading-tight" style={{ fontFamily: 'Georgia, serif' }}>
            {activeChapter.title}
          </h2>
          ...
          {/* Heart Divider */}
          <div className="mt-8 pt-6 border-t border-pink-100 flex items-center justify-center gap-2 text-pink-400">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-4 mt-8 animate-fade-in">
            <button
              ...
              className="p-2 rounded-full bg-white/20 text-white hover:bg-white/30 transition disabled:opacity-30 disabled:cursor-not-allowed"
            >
        ...
        <div className="text-center mt-8 animate-fade-in">
          <div className="flex items-center justify-center gap-2 text-white/60 text-sm">
            ...
            <span>A story written with love</span>
            <Heart className="h-4 w-4 fill-current" />
          </div>
