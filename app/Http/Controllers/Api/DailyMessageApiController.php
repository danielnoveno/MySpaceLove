...
class DailyMessageApiController extends Controller
{
    ...
    public function getTodayMessage(Space $space)
    {
        ...
        $now = $this->currentDailyMessageNow();
        $message = $this->ensureDailyMessageFor($space, $partner, $now);
    ...
    private function ensureDailyMessageFor(Space $space, User $user, Carbon $now, bool $force = false): ?DailyMessage
    {
        ...
        return $this->persistDailyMessage($space, $user, $date, $text, 'ai');
    }
    ...
    private function persistDailyMessage(
        ...
        try {
            return DailyMessage::firstOrCreate(
                ['space_id' => $space->id, 'user_id' => $user->id, 'date' => $date],
    ...
    private function recentMessagesForWeek(Space $space, string $targetDate, ?User $user = null): array
    {
